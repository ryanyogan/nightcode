...
const app = new Hono()
  .post("/:sessionId/resume", async (c) => {
    ...
    const session = await db.session.findUnique({
      where: { id: sessionId },
      include: { messsages: { orderBy: { createdAt: "asc" } } },
    });
    ...
    const resumableMessage = getResumableUserMessage(session.messsages);
    if (!resumableMessage) {
      return c.json(
    ...
    const history = buildConversationHistory(session.messsages);
